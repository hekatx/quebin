...
export class AnimatedBlur {
	...
	static getBlurStyle() {
		...
		var bodyStyle =
			...
			"} ";
		var animatedBlur = ".animated-blur { " + "  position: relative; " + "} ";
		var clonedElement =
			".clonedElement { " +
			"  position: absolute; " +
			"  display: block; " +
		...
		var composited =
			...
			"} ";
		return [html, bodyStyle, animatedBlur, composited, clonedElement];
	}
	...
	calculateMargin() {
		this.marginTop = parseInt(window.getComputedStyle(this.element).marginTop);
		this.marginLeft = parseInt(
			window.getComputedStyle(this.element).marginLeft,
		);
		if (this.marginTop == 0) {
			...
			for (var i = 0; i < descendants.length; ++i) {
				if (window.getComputedStyle(descendants[i]).marginTop != "0px") {
					this.marginTop = parseInt(
						window.getComputedStyle(descendants[i]).marginTop,
					);
					break;
				}
			}
	...
	cloneElements() {
		...
		for (var i = 1; i <= this.num; ++i) {
			...
			if (i == 1) {
				...
				clone.childNodes[1].style.filter = "blur(" + filterStdDev + "px)";
				filterStdDev *= 2;
			}
	...
	resize() {
		...
		for (var i = 0; i < elements.length; ++i) {
			elements[i].style.top = this.element.offsetTop - this.marginTop + "px";
			elements[i].style.left = this.element.offsetLeft - this.marginLeft + "px";
