...
type Posts = ReturnType<typeof getAllPosts>;
...
function Post({ post }: { post: Posts[number] }) {
  ...
  return <article className="mb-6 animate-text" >
    <Link href={`/blog/${slug}`}>
    ...
    <div className="flex gap-3">
      ...
      <div className="flex gap-3">{tags.map(tag => <Tag key={tag}>{tag}</Tag>)}</div>
    </div>
  </article>
}

function Tag({ children }: { children: ReactNode }) {
  return <span className="p-1 bg-zinc-800 text-xs rounded-sm tracking-wider">{children}</span>
}
